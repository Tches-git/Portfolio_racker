...
项目已从单一“研报生成工具”升级为本地单用户金融消息追踪平台：

- **全中文报告中心**：首页按“组合、事件、预警、简报、研报交付、运行审计”组织，提供核心指标、常用报告入口、事件明细表和待办状态栏。
- **组合风险驾驶舱**：Watchlist 不再只是股票列表，而是组合级事件、预警、风险分、处理率、优先动作和受影响股票排序。
- **预警处理闭环**：内置只读规则中心，支持预警筛选、事件状态处理、处理人/时间/备注记录，以及已转研报状态沉淀。
- **事件驱动研究**：单条事件可触发事件点评或更新研报，并进入统一任务中心追踪运行状态。
- **导出中心分类**：股票导出页按研报正文、事件点评、展示交付、来源数据、运行日志分类展示，复用统一下载接口。

## 项目亮点

| 能力 | 说明 |
|------|------|
| 全中文报告中心 | 以报告中心组织首页信息层级，集中展示关键指标、常用报告、事件明细和待办状态 |
| 金融事件追踪 | 聚合公告、交易所披露、实时行情、券商观点，统一为 `MarketEvent` |
| 语义去重合并 | 按 URL、标题和同日相似度合并重复消息，保留多来源证据 |
...
- `frontend/`：独立前端骨架（Next.js），当前消费兼容 API，并已补上全中文报告中心、动态 sidebar、全局运行中心、独立任务中心、事件追踪 / 事件详情 / 组合跟踪 / 预警中心 / 每日简报 / 股票工作台等页面，以及浏览器触发分析与状态轮询入口
- 事件追踪接口包括 `/api/v1/events`、`/api/v1/events/{event_id}`、`/api/v1/events/{event_id}/analyze`、`/api/v1/watchlists`、`/api/v1/alerts`、`/api/v1/alerts/rules`、`/api/v1/briefing/daily`
- CLI/API 仍支持同时导出 Markdown / HTML 展示版 / 来源索引 JSON；环境已安装 `reportlab` 且可注册中文字体时，会额外生成 PDF 展示版
- 股票导出中心会合并最新报告和最近任务导出物，并按研报正文、事件点评、展示交付、来源数据和运行日志分类
